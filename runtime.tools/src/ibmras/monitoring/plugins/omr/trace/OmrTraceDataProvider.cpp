...
#include "ibmras/monitoring/plugins/omr/trace/OmrTraceDataProvider.h"
#include "omragent.h"
...
IBMRAS_DEFINE_LOGGER("OmrTraceDataProvider");
...
pushsource* registerPushSource(void (*callback)(monitordata* data),
	...
	ibmras::monitoring::plugins::omr::trace::provID = provID;
	ibmras::monitoring::plugins::omr::trace::sendDataToAgent = callback;

//	outfile = fopen("/home/corbin/trace.out", "wb");
//	if (NULL == outfile) {
//		fprintf(stderr, "Failed to opehe is here nown file\n");
//	}
//
//	tobesoutfile = fopen("/home/corbin/tobestrace2.out", "wb");
//	if (NULL == tobesoutfile) {
...
int Tracestart() {
	...
	if (OMR_ERROR_NONE == rc) {
		rc = vmData.omrti->BindCurrentThread(vmData.theVm, &vmThread);
	}
...
OmrTraceDataProvider::OmrTraceDataProvider(
