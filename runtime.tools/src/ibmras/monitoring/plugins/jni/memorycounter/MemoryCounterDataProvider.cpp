...
char* getMemoryCounters(JNIEnv *env)
{
    ...
    const char* memCounterFormatString = "memcounterinfo, %s, "JLONG_FMT_STR", "JLONG_FMT_STR", "JLONG_FMT_STR", "JLONG_FMT_STR", %ld, %ld, %ld\n";
    char buffer[1000];


    /* Find out how much categories buffer to allocate */
    err = tdpp->jvmtiGetMemoryCategories(tdpp->pti, COM_IBM_GET_MEMORY_CATEGORIES_VERSION_1, 0, NULL, NULL, &total_categories);
    if (JVMTI_ERROR_NONE != err)
    {
        goto cleanup;
    }

    categories_buffer = (jvmtiMemoryCategory*)ibmras::monitoring::plugins::jni::hc_alloc(total_categories * sizeof(jvmtiMemoryCategory));
    if (categories_buffer == NULL)
    {
        goto cleanup;
    }

    err = tdpp->jvmtiGetMemoryCategories(tdpp->pti, COM_IBM_GET_MEMORY_CATEGORIES_VERSION_1, total_categories, categories_buffer, &written_count, &total_categories);
    if (JVMTI_ERROR_NONE != err)
    {
        goto cleanup;
    }
    ...
    memcounterarray = (char**)ibmras::monitoring::plugins::jni::hc_alloc((written_count) * sizeof(char *));
    ...
    for (i = 0; i < written_count; i++)
    {
        ...
        if (NULL != categories_buffer[i].firstChild)
        {
            ...
            {
                firstChildNumber = firstChildNumber / sizeof(jvmtiMemoryCategory);
            }
        } else
        {
            firstChildNumber = -1;
        ...
        if (NULL != categories_buffer[i].nextSibling)
        {
            ...
            {
                nextSiblingNumber = nextSiblingNumber / sizeof(jvmtiMemoryCategory);
            }
        } else
        {
            nextSiblingNumber = -1;
        ...
        memcounterarray[i] = (char *)ibmras::monitoring::plugins::jni::hc_alloc(strlen(buffer) + 1);
    ...
    finalReport = ibmras::monitoring::plugins::jni::join_strings(memcounterarray, i);


    cleanup:
    ibmras::monitoring::plugins::jni::hc_dealloc((unsigned char**)&category_name);
    ibmras::monitoring::plugins::jni::hc_dealloc((unsigned char**)&categories_buffer);
    ibmras::monitoring::plugins::jni::dealloc_report_lines(memcounterarray, written_count);
    ibmras::monitoring::plugins::jni::hc_dealloc((unsigned char**)&memcounterarray);
	...
	ibmras::monitoring::plugins::jni::hc_dealloc((unsigned char**)&finalReport);
