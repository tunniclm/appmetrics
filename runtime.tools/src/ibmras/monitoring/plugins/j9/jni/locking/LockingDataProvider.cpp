...
monitordata* JLAPullSource::sourceData(jvmFunctions* tdpp, JNIEnv* env) {
	...
	if (isEnabled()) {
		...
		ibmras::monitoring::plugins::j9::jni::hc_dealloc(reinterpret_cast<unsigned char**>(&value));
...
char* monitor_dump_event(JNIEnv *env) {
	...
	finalReport = (char*) ibmras::monitoring::plugins::j9::jni::hc_alloc(
			sizeof(char) * 500);
	if (finalReport == NULL) {
		goto cleanup;
	}

	reportLineArray = (char**) ibmras::monitoring::plugins::j9::jni::hc_alloc(
	...
	arrjmon = (jdata_t **) ibmras::monitoring::plugins::j9::jni::hc_alloc(
			arrj_size);
	if (arrjmon == NULL) {
		goto cleanup;
	}
	...
	arrsmon = (jdata_t **) ibmras::monitoring::plugins::j9::jni::hc_alloc(
			arrs_size);
	if (arrsmon == NULL) {
		goto cleanup;
	}
	...
	reportLineArray[reportLineCount] =
			reinterpret_cast<char*>(ibmras::monitoring::plugins::j9::jni::hc_alloc(
					strlen(buffer) + 1));
	...
	while (stt < end) {
		...
		jp = (jdata_t *) ibmras::monitoring::plugins::j9::jni::hc_alloc(
				sizeof(jdata_t));
		if (jp == NULL) {
			goto cleanup;
		}
		...
		jp->mnm = ibmras::monitoring::plugins::j9::jni::dupJavaStr(stt);
		if (jp->mnm == NULL) {
			goto returnReport;
		}
	...
	for (i = 0; i < rawcnt; i++) {
		...
		if (reportLineCount >= reportArraySize) {
			...
			reportLineArray =
					(char**) ibmras::monitoring::plugins::j9::jni::hc_realloc_ptr_array(
	...
	if (p) {
		ibmras::monitoring::plugins::j9::jni::hc_dealloc((unsigned char**) &q[0]);
		ibmras::monitoring::plugins::j9::jni::hc_dealloc((unsigned char**) &p);
	}
	...
	ibmras::monitoring::plugins::j9::jni::hc_dealloc((unsigned char**) &arrjmon);
	ibmras::monitoring::plugins::j9::jni::hc_dealloc((unsigned char**) &arrsmon);

	ibmras::monitoring::plugins::j9::jni::dealloc_report_lines(reportLineArray,
			reportArraySize);
	ibmras::monitoring::plugins::j9::jni::hc_dealloc(
