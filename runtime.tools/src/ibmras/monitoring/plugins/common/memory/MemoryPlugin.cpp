...
int MemoryPlugin::start() {
	aCF.logMessage(ibmras::common::logging::debug, ">>>MemoryPlugin::start()");
	noFailures = true;
	aCF.logMessage(ibmras::common::logging::debug, "<<<MemoryPlugin::start()");
	return 0;
}
...
int MemoryPlugin::stop() {
	aCF.logMessage(ibmras::common::logging::debug, ">>>MemoryPlugin::stop()");
	aCF.logMessage(ibmras::common::logging::debug, "<<<MemoryPlugin::stop()");
	return 0;
}

pullsource* MemoryPlugin::createSource(agentCoreFunctions aCF, uint32 provID) {
	aCF.logMessage(ibmras::common::logging::fine, "[memory_os] Registering pull source");
	if(!instance) {
...
pullsource* ibmras_monitoring_registerPullSource(agentCoreFunctions aCF, uint32 provID) {
	aCF.logMessage(ibmras::common::logging::debug, "[memory_os] Registering pull source");
...
int ibmras_monitoring_plugin_start() {
	MemoryPlugin::aCF.logMessage(ibmras::common::logging::fine, "[memory_os] Starting");
	return 0;
}

int ibmras_monitoring_plugin_stop() {
	MemoryPlugin::aCF.logMessage(ibmras::common::logging::fine, "[memory_os] Stopping");
	return 0;
}
