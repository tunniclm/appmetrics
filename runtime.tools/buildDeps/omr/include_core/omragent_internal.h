...
typedef struct OMR_PdhCounterPath {
	...
	uintptr_t counterPathStatus;					/* If counter path has been initialized or PDH query has failed */
...
typedef struct OMR_ThreadAPI {
    intptr_t (*j9thread_create)(j9thread_t *handle, uintptr_t stacksize, uintptr_t priority, uintptr_t suspend, j9thread_entrypoint_t entrypoint, void *entryarg, uint32_t category);
    intptr_t (*j9thread_monitor_init_with_name)(j9thread_monitor_t *handle, uintptr_t flags, const char *name);
    intptr_t (*j9thread_monitor_destroy)(j9thread_monitor_t monitor);
    intptr_t (*j9thread_monitor_enter)(j9thread_monitor_t monitor);
    intptr_t (*j9thread_monitor_exit)(j9thread_monitor_t monitor);
    intptr_t (*j9thread_monitor_wait)(j9thread_monitor_t monitor);
    intptr_t (*j9thread_monitor_notify_all)(j9thread_monitor_t monitor);
    intptr_t (*j9thread_attr_init)(j9thread_attr_t *attr);
    intptr_t (*j9thread_attr_destroy)(j9thread_attr_t *attr);
    intptr_t (*j9thread_attr_set_detachstate)(j9thread_attr_t *attr, j9thread_detachstate_t detachstate);
    intptr_t (*j9thread_create_ex)(j9thread_t *handle, const j9thread_attr_t *attr, UDATA suspend, j9thread_entrypoint_t entrypoint, void *entryarg, U_32 category);
    intptr_t (*j9thread_join)(j9thread_t thread);
} OMR_ThreadAPI;
