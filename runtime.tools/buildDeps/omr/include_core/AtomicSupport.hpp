...
#pragma warning(disable:4035)
	...
	{
#if !defined(ATOMIC_SUPPORT_STUB)
		/* If we aren't 64 bit (ie: uintptr_t == uint32_t) then use the 32 bit specializations */
#if !defined(J9VM_ENV_DATA64)
		return (uintptr_t)lockCompareExchangeU32((volatile uint32_t *)address, (uint32_t)oldValue, (uint32_t)newValue);
#else /* !defined(J9VM_ENV_DATA64) */

		/* These are only non 32bit uintptr_t specializations */
#if defined(_MSC_VER)
		/* Note: This did not work correctly under MSVC6 using debug.  It appeared that not all of the lock cmpxchg's were emitted.
		 * They looked to be optimized out, regardless of the lower opt level, and the volatile variables.
		 */
		return (uintptr_t)_InterlockedCompareExchange64((volatile intptr_t *)address, (intptr_t)newValue, (intptr_t)oldValue);
#elif defined(__GNUC__) && defined(J9HAMMER) /* defined(_MSC_VER) */
		 return __sync_val_compare_and_swap(address, oldValue, newValue);
#elif defined(J9ZOS39064) /* defined(__GNUC__) && defined(J9HAMMER) */
		 /* V1.R13 has a compiler bug and if you pass a constant as oldValue it will cause c-stack corruption */
		 volatile uintptr_t old = oldValue;
		 /* Call __csg directly as csg() does not exist */
		__csg((uintptr_t *)&old, (uintptr_t *)address, (uintptr_t *)&newValue);
		return old;
...
#error "lockCompareExchange(): unsupported platform!"
	...
	{
#if !defined(ATOMIC_SUPPORT_STUB)
		/* On 64-bit platforms, call the uintptr_t-sized operation. It should probably be the other
		 * way around, but it shouldn't be changed until there is a complete inline implementation
		 * here
		 */
#if defined(J9VM_ENV_DATA64)
		return lockCompareExchange((volatile uintptr_t *)address, (uintptr_t)oldValue, (uintptr_t)newValue);
