 ...
 */
J9Ranking* rankingNew(J9PortLibrary *portLibrary, uint32_t size);
...
uintptr_t rankingGetLowestCount(J9Ranking * ranking);
 ...
 */
void rankingUpdateLowest(J9Ranking * ranking, void * key, uintptr_t count);
 ...
 * */
uintptr_t rankingIncrementEntry(J9Ranking * ranking, void * key, uintptr_t count);

/* get the key at rank k
 * @param k the kth highest count we're enquiring about
 * @return the key at rank k
 */
void * rankingGetKthHighest(J9Ranking * ranking, uintptr_t k);

/* get the count at rank k
 * @param k the kth highest count we're enquiring about
 * @return the count at rank k
 */
uintptr_t rankingGetKthHighestCount(J9Ranking * ranking, uintptr_t k);
