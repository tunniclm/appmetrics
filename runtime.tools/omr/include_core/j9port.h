...
#define J9PORT_LIMIT_HARD ((UDATA) 0x80000000)
#define J9PORT_RESOURCE_SHARED_MEMORY ((UDATA) 1)
#define J9PORT_RESOURCE_ADDRESS_SPACE ((UDATA) 2)
#define J9PORT_RESOURCE_CORE_FILE ((UDATA) 3)
#define J9PORT_RESOURCE_CORE_FLAGS ((UDATA) 4)
/** @} */
 ...
 * @{
 */
#define J9PORT_TIME_DELTA_IN_SECONDS ((U_64) 1)
#define J9PORT_TIME_DELTA_IN_MILLISECONDS ((U_64) 1000)
#define J9PORT_TIME_DELTA_IN_MICROSECONDS ((U_64) 1000000)
#define J9PORT_TIME_DELTA_IN_NANOSECONDS ((U_64) 1000000000)
/** @} */
 ...
 * @{
 */
#define J9PORT_TIME_NS_PER_MS ((U_64) 1000000)	/* nanoseconds per millisecond */
#define J9PORT_TIME_US_PER_SEC ((U_64) 1000000) /* microseconds per second */
/** @} */
...
#define J9SH_DIRPERM_ABSENT ((UDATA)-2)
...
/* 
 * Flags passed to "flag" argument of j9shmem_open(). Should be of type UDATA.
...
#define J9SHMEM_STORAGE_KEY_TESTING_SHIFT	((sizeof(UDATA)*8)-4)
...
{
	U_32 isUserWriteable : 1;
	U_32 isUserReadable : 1;
	U_32 isGroupWriteable : 1;
	U_32 isGroupReadable : 1;
	U_32 isOtherWriteable : 1;
	U_32 isOtherReadable : 1;
	U_32 : 26; /* future use */
} J9Permission;
...
typedef struct  J9PortShSemParameters {
 	...
 	U_32 setSize; /* number of semaphores to be created in this set */
 	U_32 permission; /* Posix-style file permissions */
 	const char* controlFileDir; /* Directory in which to create control files (SysV semaphores only) */
 	U_8 proj_id; /* parameter used with semName to generate semaphore key */
 	U_32 deleteBasefile : 1; /* delete the base file (used to generate the semaphore key) when destroying the semaphore */
 ...
 */
#define J9PORT_PROCESS_STDIN ((UDATA) 0x00000001)
#define J9PORT_PROCESS_STDOUT ((UDATA) 0x00000002)
#define J9PORT_PROCESS_STDERR ((UDATA) 0x00000004)
...
typedef struct J9SysinfoEnvIteratorState {
	...
	UDATA bufferSizeBytes;  /* size of @ref buffer */
...
typedef struct J9MemCategory {
	...
	const U_32 categoryCode;
	UDATA liveBytes;
	UDATA liveBytesLockWord;
	UDATA liveAllocations;
	UDATA liveAllocationsLockWord;
	const U_32 numberOfChildren;
	const U_32 * const children;
} J9MemCategory;

typedef struct J9MemCategorySet {
	U_32 numberOfCategories;
...
typedef struct J9MemCategoryWalkState {
	...
	*/
	UDATA (*walkFunction) (U_32 categoryCode, const char * categoryName, UDATA liveBytes, UDATA liveAllocations, BOOLEAN isRoot, U_32 parentCategoryCode, struct J9MemCategoryWalkState * state);
...
{
	UDATA j9NodeNumber;  /**< The 1-indexed number used outside of Port and Thread to describe this node */
	...
	UDATA computationalResourcesAvailable;  /**< The number of computational resources on this node which are currently available for use under the current NUMA policy */
} J9MemoryNodeDetail;
...
typedef struct J9MemoryInfo {
	U_64 totalPhysical;		/* Total physical memory in the system (in bytes). */
	U_64 availPhysical;		/* Available physical memory in the system (in bytes). */
	U_64 totalVirtual;		/* Total virtual memory addressable by the process (in bytes). */
	U_64 availVirtual;		/* Virtual memory available to the process (in bytes). */
	U_64 totalSwap;			/* Total swap memory (in bytes). */
	U_64 availSwap;			/* Total swap memory free (in bytes). */
	U_64 cached;			/* The physical RAM used as cache memory (in bytes). */
	U_64 buffered;			/* The physical RAM used for file buffers (in bytes). */
	I_64 timestamp;			/* Sampling timestamp (in microseconds). */
} J9MemoryInfo;

#define J9PORT_MEMINFO_NOT_AVAILABLE ((U_64) -1)
...
typedef struct J9ProcessorInfo {
	U_64 userTime;			/* Time spent in user mode (in microseconds). */
	U_64 systemTime;		/* Time spent in system mode (in microseconds). */
	U_64 idleTime;			/* Time spent sitting idle (in microseconds). */
	U_64 waitTime;			/* Time spent over IO wait (in microseconds). */
	U_64 busyTime;			/* Time spent over useful work (in microseconds). */
	I_32 proc_id;			/* This processor's id. */
...
typedef struct J9ProcessorInfos {
	I_32 totalProcessorCount;			/* Number of logical processors on the machine. */
	J9ProcessorInfo *procInfoArray;		/* Array of processors, of 'totalProcessorCount + 1'. */
	I_64 timestamp;						/* Sampling timestamp (in microseconds). */
} J9ProcessorInfos;

#define J9PORT_PROCINFO_NOT_AVAILABLE ((U_64) -1)

/* Processor status. */
#define J9PORT_PROCINFO_PROC_OFFLINE ((I_32)0)
#define J9PORT_PROCINFO_PROC_ONLINE ((I_32)1)
...
typedef struct J9VirtReadWriteLock {
	U_64 unused1[2];
	spinlock_t spinlock;
	volatile U_32 lock_users; /* Reader-count */
	volatile U_32 lock_flags;
	volatile U_32 lock_status;
	U_64 lock_word; /* Argument #4 to compareSwap*(). */
	volatile I_32 locking_guest_id;
	volatile I_32 locking_jvm_pid;
	U_32 unused2;
	U_32 unused3;
	U_64 unused4;
} J9VirtReadWriteLock;
...
typedef struct J9VirtConfig {
	U_64 sharedObjectSize;
	IDATA hypervisorKind;
	char sharedObjectName[J9HYPERVISOR_MAX_SHARED_OBJECT_PATH_LEN];
	IDATA guestId;
	IDATA initialized;
	I_32 guestMemUsed;
	I_32 guestMemUseThreshold;
	I_32 currentMemUseThreshold;
	U_64 guestCpuUsed;
	U_64 guestCpuUseThreshold;
	U_64 currentCpuUseThreshold;
...
typedef struct J9ProcessorDesc {
	...
	U_32 features[J9PORT_SYSINFO_FEATURES_SIZE];
} J9ProcessorDesc;
